document.addEventListener('DOMContentLoaded', () => {
    ...
    const heroSection = document.getElementById('hero-miller'); // ID CAMBIADO
    const enganoSection = document.getElementById('la-tragedia-de-miller'); // ID CAMBIADO
    const confrontacionSection = document.getElementById('el-costo-de-la-mision'); // ID CAMBIADO
    ...
    // --- LÓGICA PARA BURBUJAS (SECCIÓN 2) ---
    if (snowCanvas && caractSection) {
        ...
        function createBubbleParticle() {
            return {
                ...
                radius: Math.random() * 3 + 1, // Burbujas de varios tamaños
                color: 'rgba(173, 216, 230, 0.5)', // Azul claro y transparente
                vx: (Math.random() - 0.5) * 0.8, // Ligero balanceo horizontal
                vy: -(Math.random() * 1 + 0.5), // Movimiento constante hacia arriba
            };
        }
        ...
        function initBubbles() {
            ...
            for (let i = 0; i < snowParticleCount; i++) {
                snowParticles.push(createBubbleParticle());
            }
        }
        ...
        function animateBubbles() {
            ...
            snowParticles.forEach(p => {
                ...
                // Si la burbuja sale por arriba, reaparece por abajo
                if (p.y < -p.radius) {
                    p.y = snowCanvas.height + p.radius;
            ...
            snowAnimationId = requestAnimationFrame(animateBubbles);
        }
        ...
        bubbleObserver.observe(caractSection);
